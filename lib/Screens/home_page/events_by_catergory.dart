...
class EventByCategory extends StatefulWidget {
  ...
  const EventByCategory({super.key,required this.catergory});

  @override
  State<EventByCategory> createState() => _Test1State();
}

class _Test1State extends State<EventByCategory> {
    ...
    await FirebaseFirestore.instance
        ...
        .then(
            (snapshot) => snapshot.docs.forEach(
                (document) {
                  if(document["category"] == widget.catergory) {
        ...
        child: FutureBuilder(
            ...
            builder: (context,snapshot){
              return Expanded(
                child: Container(
                  width: 400,
                  child: (_activeDocIDs.isEmpty)
                      ?  Center(
                      child: Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: const [
                          Text(
                            'There is no event right now',
                            style: TextStyle(
                                color: Colors.white,
                                fontSize: 17
                            ),
